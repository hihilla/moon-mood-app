...
// One planet's horoscope reading: name as a heading, its theme stated once
// underneath, then its facts (house focus, natal-planet aspect) as bullets.
// Grouping this way avoids restating "Your Sun" as the subject of every
// line — the heading carries that once for everything below it.
export function PlanetReading({ reading }) {
    if (!reading || (!reading.houseFact && !reading.aspectFact)) return null;
    const { planet, theme, houseFact, aspectFact } = reading;
    return (
        <div className="mb-3 last:mb-0">
            <div className="text-sm font-semibold" style={{ color: COLORS.ink }}>{planet}</div>
            {theme && <div className="text-xs mb-1" style={{ color: COLORS.inkSoft }}>{theme}</div>}
            {houseFact && <div className="text-sm">• {houseFact}</div>}
            {aspectFact && <div className="text-sm mt-0.5">• {aspectFact}</div>}
        </div>
    );
}