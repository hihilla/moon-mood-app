...
//
// Returns fragments rather than full pre-built sentences (theme stated
// once, houseFact/aspectFact as standalone clauses) so a UI can group
// everything under one heading per planet instead of repeating "Your Sun"
// as the subject of every line.
//
...
export function buildFullHoroscope({ moonLongitude, transitPlanets, natalPlanets, natalHouses }) {
  ...
  return allTransits
      ...
      .map((p) => {
        const theme = PLANET_THEME[p.name];
        const house = houseForLongitude(p.longitude, natalHouses);
        // A standalone clause, not a full sentence — the planet + its theme
        // is stated once by the caller (as a heading), not repeated here.
        const houseFact = house ? `Focused on ${HOUSE_MEANING[house]}.` : null;
        ...
        // A DIFFERENT fact from houseFact above: not "where this planet is,"
        // but "is it forming an angle with one of your other natal planets
        // today." Naming the natal planet keeps the two facts distinct even
        // without a repeated subject.
        const aspectFact = topAspect
            ? `${capitalize(ASPECT_TONE[topAspect.type])} your natal ${topAspect.natalPlanet} — ${targetTheme}.`
            : null;

        return { planet: p.name, theme, house, houseFact, topAspect, aspectFact };
      });
}
