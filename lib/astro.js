...
export function buildFullHoroscope({ moonLongitude, transitPlanets, natalPlanets, natalHouses }) {
  ...
  return allTransits
      ...
      .map((p) => {
        ...
        const houseLine = house
            ? `Transiting ${p.name === "Moon" ? "moon" : p.name} is moving through your ${ordinal(house)} house — ${HOUSE_MEANING[house]}.`
            : null;

        const natalMatches = (natalPlanets || [])
            .map((np) => {
        ...
        const targetTheme =
            topAspect && (PLANET_THEME[topAspect.natalPlanet] || topAspect.natalPlanet);
