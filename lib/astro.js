...
export function buildFullHoroscope({ moonLongitude, transitPlanets, natalPlanets, natalHouses }) {
  ...
  return allTransits
      ...
      .map((p) => {
        ...
        // States what this planet's energy actually IS (its plain-language
        // theme) and what life area it's currently pointed at — in one
        // sentence, so the reader never has to infer the connection between
        // "a planet is in a house" and "what that means" themselves.
        const houseLine = house
            ? `Your ${p.name} — your ${PLANET_THEME[p.name]} — is focused on ${HOUSE_MEANING[house]}.`
            : null;
        ...
        // A DIFFERENT fact from the house line above: not "where this planet
        // is," but "is it forming an angle with one of your other natal
        // planets today." Repeating the planet's name as the subject of both
        // sentences (rather than a filler transition word) is what keeps
        // this from reading as a continuation of the house sentence.
        const aspectLine = topAspect
            ? `Your ${p.name} is ${ASPECT_TONE[topAspect.type]} your natal ${topAspect.natalPlanet} — ${targetTheme}.`
            : null;
